...
class Visualizer:
    def __init__(self, grid_width, grid_height, obstacles, solutions):
    ...
    def choose_file(self):
        """Cho phép người dùng chọn file input mới và cập nhật giao diện."""
        file_path = askopenfilename(
            title="Chọn tệp đầu vào",
        ...
        if file_path:
            ...
            # Khởi tạo lại các thông tin liên quan đến CBS
            cbs = HighLevelCBS()
            cbs.read_input(file_path)
            solution = cbs.run_cbs()
            if solution is None:
            ...
            # Làm mới giao diện
            self.canvas.delete("all")  # Xóa toàn bộ nội dung canvas
            self.setup_grid()          # Vẽ lưới mới
            self.setup_agents_and_goals()  # Vẽ agent và điểm đích
