...
from ICTS import HighLevelICTS
from CBSData import Path, Vertex

class Visualizer:
    def __init__(self, grid_width, grid_height, obstacles, solutions, file_path, algo):
        ...
        self.algo = algo  # Thuật toán được chọn (CBS hoặc ICTS)
    ...
    def choose_file(self):
        ...
        if file_path:
            ...
            if self.algo:  # Nếu thuật toán đã được chọn trước đó, chạy lại với file mới
                self.algo.read_input(self.file_path)
                if self.algo == HighLevelCBS:
                    self.solutions = self.algo.run()
                else:
                    self.solutions = self.algo.run()
                #self.solutions = self.algo.run_cbs() if isinstance(self.algo, HighLevelCBS) else self.algo.run_icts()
            else:
                print("Vui lòng chọn thuật toán trước.")
            ...
            # Cập nhật trực quan
            self.grid_width = self.algo._lowLevelSolver._gridWidth
            self.grid_height = self.algo._lowLevelSolver._gridHeight
            self.obstacles = [(v.x, v.y) for row in self.algo._lowLevelSolver.map for v in row if v.Obstacle]
    ...
    def choose_algorithm(self, algorithm):
        """Chọn thuật toán nhưng giữ nguyên file đầu vào."""
        self.algo = HighLevelCBS() if algorithm == "CBS" else HighLevelICTS()

        if not self.file_path:  # Kiểm tra xem đã có file đầu vào chưa
            print("Không có tệp đầu vào! Vui lòng chọn tệp trước khi chạy thuật toán.")
            return

        print(f"Sử dụng file đầu vào: {self.file_path} với thuật toán {algorithm}")

        self.algo.read_input(self.file_path)
        self.solutions = self.algo.run() if algorithm == "CBS" else self.algo.run()
