...
def main():
    try:
        ...
        '''
        # Trích xuất thông tin để trực quan hóa
        grid_width = icts._lowLevelSolver._gridWidth
        grid_height = icts._lowLevelSolver._gridHeight
        obstacles = [(v.x, v.y) for row in icts._lowLevelSolver.map for v in row if v.Obstacle]

        # Tạo và hiển thị hoạt ảnh
        visualizer = Visualizer(grid_width, grid_height, obstacles, solution, input_file, icts)
